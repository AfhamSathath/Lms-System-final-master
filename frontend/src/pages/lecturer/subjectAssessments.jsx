...
import { FiClipboard, FiPlus, FiSave, FiAlertCircle, FiSend, FiUsers, FiTrash2, FiEdit2 } from 'react-icons/fi';
...
const SubjectAssessments = () => {
  ...
  const filteredAssessments = filterBatch === 'All' 
    ? assessments 
    : assessments.filter(a => a.batch === filterBatch);
         ...
         <div className="bg-white p-6 rounded-3xl shadow-xl border border-slate-100 mb-8">
                             ...
                             <td className="p-3 text-right">
                                <input 
                                   ...
                                   max={activeAssessment.maxMarks}
                                    className={`border rounded-lg py-1 px-3 w-24 text-center font-black focus:ring-indigo-500 outline-none ${
           ...
           <div className="flex justify-end gap-4 mt-8 pt-4 border-t">
              ...
              <button 
                 ...
                 className="px-8 py-2.5 font-bold text-white bg-indigo-600 hover:bg-indigo-700 rounded-xl shadow-lg flex items-center gap-2"
                 disabled={activeAssessment.status === 'published' || activeAssessment.status === 'approved_by_hod'}
              >
                 <FiSave /> {activeAssessment.status === 'draft' ? "Save Marks" : "View Only (Locked)"}
              </button>
           </div>
         </div>
      ) : (
               ...
               <div key={assess._id} className="bg-white p-6 rounded-3xl shadow-md border border-slate-100 flex flex-col justify-between">
                  <div>
                     <div className="flex justify-between items-start mb-2">
                        ...
                        <span className={`px-3 py-1 text-[10px] uppercase tracking-widest font-bold rounded-lg ${
                           assess.status === 'draft' ? 'bg-slate-100 text-slate-500' :
