  ...
  return (
    <div className="p-6 bg-white min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        {stats.map((stat, index) => (
          <motion.div
            ...
            className="bg-white p-6 rounded-3xl shadow-sm border border-black flex items-center gap-4"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white p-8 rounded-3xl shadow-sm border border-black">
          ...
          <div className="space-y-6">
            {departments.length > 0 ? (
              departments.map((dept, i) => (
                <div key={dept._id || i} className="border-b border-black pb-4 last:border-0">
                  <div className="flex justify-between mb-2">
                    <div>
                      <span className="font-bold text-gray-700">{dept.name}</span>
                      <span className="text-sm text-gray-400 ml-2">({dept.code})</span>
                    </div>
                    <span className="text-sm bg-white border border-black text-slate-700 font-bold px-2 py-1 rounded-full">{dept.stats?.students || 0} Students</span>
                  </div>
