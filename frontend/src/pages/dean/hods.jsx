...
const DeanHods = () => {
  ...
  const filteredHods = hods.filter(hod => {
    let hodFaculty = hod.faculty;
    if (!hodFaculty) {
      for (const [fac, depts] of Object.entries(facultyData)) {
        if (depts.includes(hod.department)) {
          hodFaculty = fac;
          break;
        }
      }
    }
    const matchesFaculty = user?.faculty ? hodFaculty === user.faculty : true;
...
const AppointHODModal = ({ isOpen, onClose, onSuccess, editingHod, user }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    let selectedFaculty = user?.faculty;
    if (!selectedFaculty) {
        for (const [fac, depts] of Object.entries(facultyData)) {
    ...
    try {
      if (editingHod) {
        await api.put(`/api/auth/users/${editingHod._id}`, {
          department,
          faculty: selectedFaculty
        });
        toast.success('HOD record updated');
      } else {
        await api.put(`/api/auth/users/${selectedLecturer._id}`, {
          role: 'hod',
          department,
          faculty: selectedFaculty
        });
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            >
              ...
              {user?.faculty && facultyData[user.faculty] ? (
                <optgroup label={user.faculty}>
                  {facultyData[user.faculty].map(dept => (
